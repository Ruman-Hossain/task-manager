...
exports.create = async (req, res) => {
  try {
    const user_id = "63fae1fa367b908c7d80482e";
    const { name } = req.body;
    if (!name) {
...
exports.update = async (req, res) => {
  try {
    ...
    Categories.updateOne(query, reqBody, (error, data) => {
      if (error) {
        res.status(400).json({ status: "Category Update Failed", data: error });
      } else {
        res.status(200).json({ status: "Category Update Successful", data: data });
      }
    });
  } catch (error) {
  ...
  try {
     ...
     Categories.deleteOne(query, (error, data) => {
       if (error) {
         res
           .status(400)
           .json({ status: "Category Delete Failed", data: error });
       } else {
         res
           .status(200)
           .json({ status: "Category Delete Successful", data: data });
       }
     });
